...
export async function fetchAvailabilityData(
  ...
  const run = async (t: TxDb): Promise<AvailabilityRawData | null> => {
    // Q1: salón
    const salon = (
      await t
        .select({
      ...
      await t
        ...
        .where(
          and(
            ...
            eq(services.salon_id, args.salonId),
          ),
        )
        .limit(1)
    )[0]
    ...
    const workingHoursRows = await t
      .select({
  ...
  return tx ? run(tx) : withTenant(args.salonId, run)
}
